...
"""
Plex MCP Agent
Connects to the lordraw/plex-mcp Docker MCP server.
...
class MCPClient:
    ...
    def initialize(self) -> dict:
        result = self._rpc("initialize", {
            ...
            "capabilities": {"tools": {}},
            "clientInfo": {"name": "plex-mcp-agent", "version": "1.0.0"},
        })
...
def start_docker() -> subprocess.Popen:
    image = _e("DOCKER_IMAGE", "lordraw/plex-mcp:latest")
    plex_url = _e("URL", "")
    plex_token = _e("TOKEN", "")

    if not plex_url:
        raise ValueError("PLEX_MCP_URL is required (e.g. http://192.168.0.x:32400)")
    if not plex_token:
        raise ValueError("PLEX_MCP_TOKEN is required (Plex authentication token)")

    cmd = [
        "docker", "run", "--rm", "-i",
        "-e", f"PLEX_SERVER_URL={plex_url}",
        "-e", f"PLEX_TOKEN={plex_token}",
        image,
    ]
...
def main() -> None:
    ...
    plex_url = _e("URL", "(not configured)")
    print(f"[*] Plex MCP Agent — provider: {provider} — model: {model}")
    ...
    try:
        ...
        system_prompt = (
            "You are a Plex Media Server assistant. "
            f"Use the available tools to manage and query the Plex server at {plex_url}. "
            "You can browse and search libraries, inspect media items, check active playback "
            "sessions, manage playlists, list users, and retrieve server information. "
