...
"""
Zabbix MCP Agent
Connects to the lordraw/zabbix-mcp Docker MCP server.
...
class MCPClient:
    ...
    def initialize(self) -> dict:
        result = self._rpc("initialize", {
            ...
            "capabilities": {"tools": {}},
            "clientInfo": {"name": "zabbix-mcp-agent", "version": "1.0.0"},
        })
...
def start_docker() -> subprocess.Popen:
    image = _e("DOCKER_IMAGE", "lordraw/zabbix-mcp:latest")
    zabbix_url = _e("URL", "")
    if not zabbix_url:
        raise ValueError("ZABBIX_MCP_URL is required (e.g. http://192.168.0.x/zabbix)")

    # Auth: token takes priority over user/password when both are set
    zabbix_token = _e("TOKEN", "")
    zabbix_user = _e("USER", "")
    zabbix_password = _e("PASSWORD", "")
    if not zabbix_token and not (zabbix_user and zabbix_password):
        raise ValueError(
            "Zabbix auth required: set ZABBIX_MCP_TOKEN (recommended) "
            "or both ZABBIX_MCP_USER and ZABBIX_MCP_PASSWORD"
        )

    cmd = ["docker", "run", "--rm", "-i",
           "-e", f"ZABBIX_URL={zabbix_url}"]

    if zabbix_token:
        cmd += ["-e", f"ZABBIX_TOKEN={zabbix_token}"]
    if zabbix_user:
        cmd += ["-e", f"ZABBIX_USER={zabbix_user}"]
    if zabbix_password:
        cmd += ["-e", f"ZABBIX_PASSWORD={zabbix_password}"]

    verify_ssl = _e("VERIFY_SSL", "")
    if verify_ssl:
        cmd += ["-e", f"ZABBIX_VERIFY_SSL={verify_ssl}"]
...
def main() -> None:
    ...
    zabbix_url = _e("URL", "(not configured)")
    auth_mode = "token" if _e("TOKEN") else "user/password"
    print(f"[*] Zabbix MCP Agent — provider: {provider} — model: {model}")
    print(f"[*] Zabbix URL: {zabbix_url}  auth: {auth_mode}")
    ...
    try:
        ...
        system_prompt = (
            "You are a Zabbix infrastructure monitoring assistant. "
            f"Use the available tools to monitor and query the Zabbix server at {zabbix_url}. "
            "You can list monitored hosts, check active problems and triggers, inspect events "
            "and alerts, query item metrics, and manage maintenance windows. "
